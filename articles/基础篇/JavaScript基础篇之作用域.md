...
function foo() {
    var b = 'bbb';
    console.log(a); // 结果：aaa。说明 内层作用域 可以访问 外层作用域 里的变量
}
...
结果：undefined。注意，打印结果并没有报错，而是 undefined，说明变量 a 被提前声明了，只是尚未被赋值。
...
结果：undefined。注意，打印结果并没有报错，而是 undefined。这个例子，再次说明了：变量 i 在函数执行前，就被提前声明了，只是尚未被赋值。
...
因为此时foo被声明了（这里只是变量声明），且为undefined，并没有把 `function(){}` 赋值给 foo。


## 函数作用域

**注意**：在函数作用域中，也有声明提前的特性：
